...
def check_paths_lines(obj, line=2):
...
def read_config(cfg):
    ...
    for section in config.sections():
        ...
        for option in config.options(section):
            value = config.get(section, option).strip().encode('utf-8').decode('utf-8')
...
def stat_max_count(klist):

    max_key = ""
    max_count = 0

    for i in set(klist):
        kcount = klist.count(i)
        if kcount >= max_count:
            max_count = kcount
            max_key = i
...
def read_sample_file(input):
    ...
    with open(input, "r") as fr:
        for line in fr:
            line = line.strip()
            prefixs.append(line.split('\t')[0])
            reads1.append(line.split('\t')[1])
            reads2.append(line.split('\t')[2])
    return prefixs, reads1, reads2
