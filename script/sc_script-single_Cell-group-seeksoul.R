...
', config$`Others File`$Cell2editing, ' \
  ...
  script_2 <- paste0('for j in $(cat ', result_dir, '/', celltype, '/chromosome.txt | awk \'{print $1}\'); do
  (samtools view -H "', result_dir, '/', celltype, '/', celltype, '.marked_duplicates.${j}.bam";
   samtools view "', result_dir, '/', celltype, '/', celltype, '.marked_duplicates.${j}.bam" | awk -v chr="$j" \'$3 == chr\') |
   samtools view -bS - > "', result_dir, '/', celltype, '/', celltype, '.marked_duplicates2.${j}.bam"
   echo "', result_dir, '/', celltype, '/', celltype, '.marked_duplicates2.${j}.bam" >> ./merge.path2
done
rm -rf ', result_dir, '/', celltype, '/', celltype, '.marked_duplicates.*.bam
samtools merge -b merge.path2 ', result_dir, '/', celltype, '/', celltype, '.marked_duplicates.bam
picard SortSam -I ', result_dir, '/', celltype, '/', celltype, '.marked_duplicates.bam -O ', result_dir, '/', celltype, '/', celltype, '.marked_duplicates.sort.bam -SORT_ORDER coordinate
samtools index ', result_dir, '/', celltype, '/', celltype, '.marked_duplicates.sort.bam')

  script2 <- paste0('samtools index ', result_dir, '/', celltype, '/', celltype, '.marked_duplicates.bam
python ', config$`Others File`$reditools, ' \
        -S \
        -q 25 \
        -mbp 6 \
        -Mbp 6 \
        -f ', result_dir, '/', celltype, '/', celltype, '.marked_duplicates.sort.bam \
        -r  ', config$`Others File`$ref_genome, '  \
        -o ', result_dir, '/', celltype, '/REDItools_serial_table.txt')
  # Split the script into lines
  lines <- unlist(strsplit(script, "\n"))
  lines_2 <- unlist(strsplit(script_2, "\n"))
  ...
  modified_script_2 <- paste(lines_2, sep = "\n")
  ...
  cat(modified_script_2, file = file_path, append = TRUE, sep = "\n")
  # 追加写入第二个脚本内容
