...
option_list <- list(
  ...
  make_option(c("-w", "--work"), type = "character", help  = "Cellranger Work dir"),
...
# 获取当前项目src配置文件(不能动)
#source("/mnt/sda/project/yjc1/pipeline/02_scRNA_edit/script/src.R")
source(args$src)
...
lapply(dir_list, function(x){if (!dir.exists(paste(sep = "/", out_dir, x))) {dir.create(paste(sep = "/", out_dir, x), recursive = T)}})
out <- list()
out[['dir']] <- out_dir
out[['work']] <- work_dir
...
#print(head(human_common_marker_list))
...
seurat_deal_filter <-
  subset(seurat_deal,
         subset = nFeature_RNA > num_feature_low_threshold &
           nFeature_RNA < num_feature_high_threshold & percent.mt < mt_gene_threshold & Dissociation_reaction_marker1 < x_95)
...
rna_edit_out <- paste0(dirname(out$dir), '/04_RNAedit')
if (!dir.exists(rna_edit_out)){
