...
def work_after_anno(work_dir, job_type, out_dir):
    ...
    task = Task(
        ...
        option="-pe smp 4",
        script="""
{Rscript} {out_dir}/step2_afterAnn.R 
...
def run_after_anno(job_type, concurrent, refresh, work_dir, out_dir):

    work_dir = mkdir(work_dir)
    out_dir = mkdir(out_dir)

    dag = DAG("work_after_anno")

    after_anno_task  = work_after_anno(
        work_dir = mkdir(os.path.join(work_dir, "03_scdata")),
        out_dir = out_dir,
        job_type = job_type
    )

    dag.add_task(after_anno_task)
...
def add_after_anno_args(parser):
...
contact:  %s <%s>\
    ...
    parser = add_after_anno_args(parser)
